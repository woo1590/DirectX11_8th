...
static int AppendExtensionToSpecBuf( const char *ext, char *specBuf, size_t specBufLen )
{
    ...
    if ( strlen(specBuf) > 0 )
    {
        strncat_s( specBuf, specBufLen - strlen(specBuf) - 1,SEP, specBufLen - strlen(specBuf) - 1 );
    ...
    strncat_s(specBuf, specBufLen - strlen(specBuf) - 1, extWildcard, specBufLen - strlen(specBuf) - 1);
